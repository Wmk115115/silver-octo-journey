...
void ReadGrade()
{
	FILE* pf = fopen("D:\\desktop\\Code-dev\\wenjian\\俄罗斯方块最高得分记录.txt", "r"); //以只读方式打开文件
	if (pf == NULL) //打开文件失败
	{
		pf = fopen("D:\\desktop\\Code-dev\\wenjian\\俄罗斯方块最高得分记录.txt", "w"); //以只写方式打开文件（文件不存在可以自动创建该文件）
...
void WriteGrade()
{
	FILE* pf = fopen("D:\\desktop\\Code-dev\\wenjian\\俄罗斯方块最高得分记录.txt", "w"); //以只写方式打开文件
	if (pf == NULL) //打开文件失败
	{
		printf("保存最高得分记录失败\n");
		exit(0);
